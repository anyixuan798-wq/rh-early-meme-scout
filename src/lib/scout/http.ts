...
export async function fetchJson<T>(
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      ...
      // Blockscout (behind Cloudflare) intermittently 500s on heavy log
      // endpoints; a scan must survive a 1-3s backend blip.
      if (res.status >= 500) {
        last = new HttpError(res.status, `${res.status} ${url}`);
        await sleep(700 * (i + 1));
        continue;
      }
      if (!res.ok) {
